...
//			Matcher  matcher = pattern.matcher(usr.getEmailId());  
		...
		public String Register(@ModelAttribute User usr,ModelMap models,BindingResult result,HttpServletRequest request)
		{
			/*String emailPattern = "^[_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@"  
					   + "[A-Za-z0-9-]"; */ 
//			+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$
...
//			Matcher  matcher = pattern.matcher(usr.getEmailId());  
			...
			else if(userDao.checkIfEmailExists(usr.getEmailId()))
			{
				...
				usr=userDao.registerUser(usr);
				request.getSession().setAttribute("LoggedInUserInformation",usr );
			
			}
			
			return "redirect:/student/addNewApplication.html";
		}
